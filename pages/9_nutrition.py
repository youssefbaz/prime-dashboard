...
def generate_meal_plan(api_key, target_cal, protein_g, goal_type, allergies, diet, custom_exclude):  # noqa: ARG001 — api_key kept for signature compat
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for meal in plan.get("meals", []):
                meal["image_url"] = search_food_image(meal.get("name", ""))
            return plan
        else:
...
with tab_planner:
    ...
    # ════════════════════════════════════════════════════════
        ...
        if not CLAUDE_KEY:
            st.warning("No Claude API key. Add ANTHROPIC_API_KEY to .streamlit/secrets.toml")
